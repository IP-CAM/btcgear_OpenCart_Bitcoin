...
class ControllerPaymentBitcoin extends Controller {
	...
	public function checkUpdate() {
		if (extension_loaded('curl')) {
			...
			$last_time = date_parse_from_format($format,$last_string);//strptime($format,$last_string);
			$current_time = date_parse_from_format($format,$current_string);//strptime($format,$current_string);
			...
			else if($last_time['day'] != $current_time['day']) {
				$this->runUpdate();
			}
			else if($last_time['hour'] != $current_time['hour']) {
				$this->runUpdate();
			}
			else if(($last_time['minute']*60)+$last_time['second'] + $num_seconds < ($current_time['minute'] * 60) + $current_time['second']) {
				$this->runUpdate();
			}
	...
	public function runUpdate() {
		...
		//bitcoin live rate API from blockchain
		$btc_ticker_url    = "http://blockchain.info/ticker"; 
		$ch 			   = curl_init( $btc_ticker_url );
		curl_setopt( $ch, CURLOPT_FOLLOWLOCATION, 1);
		curl_setopt( $ch, CURLOPT_HEADER, 0);
		curl_setopt( $ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
		$btc_response_sent = curl_exec($ch);
		curl_close($ch);
		$rate_out = json_decode($btc_response_sent);
		$btc_rate = $rate_out->$default_currency_code->last;
